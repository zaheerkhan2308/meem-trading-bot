...
import type { Portfolio, Trade, Watchlist } from '../types/api'
...
export function useMeemSocket(setControls: (controls: { marketOpen?: boolean | null; lastScan?: string | null; killSwitch?: boolean; dryRun?: boolean; circuitBreaker?: string | null }) => void) {
    ...
    const connect = () => {
      ...
      socket.onmessage = ({ data }) => {
        ...
        if (event.type === 'trade_event' && event.trade) {
          client.setQueryData<{ trades: Trade[] }>(['trades'], old => ({
